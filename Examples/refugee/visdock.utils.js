...
createPolygon.prototype.intersectPath = function(shape, inclusive) {
	...
	for ( u = 0; u < shape.length; u++) {
		...
		//var k = 0;
		while (k == 0){
			if (node.getAttributeNS(null, "transform") != ""){
				var t = node.getAttributeNS(null, "transform").split("(")[1].split(")")[0].split(",");
...
/*				var shapebound = document.createElementNS("http://www.w3.org/2000/svg", "polygon");
		...
		if (k == 2){
			...
			for (var i = 0; i < this.points.length; i++) {
				if (i != points.length - 1) {
					strpoints = [strpoints + (this.points[i][0]-ax) + "," + (this.points[i][1]-ay) + " "];
				} else {
					strpoints = [strpoints + (this.points[i][0]-ax) + "," + (this.points[i][1]-ay) + " "];
				}
			}
			strpoints = [strpoints + (this.points[0][0]-ax) + "," + (this.points[0][1]-ay)];
			shapebound.setAttributeNS(null, "points", strpoints);			
			var shapebound2D = new Polygon(shapebound);			
		}
	...
	for ( i = 0; i < shape.length; i++) {
		...
		if (inclusive != true) {
			...
			if (result.status == "Intersection") {
				...
				if (this.shapebound2D.pointInPolygon(c) || Math.pow((cx - this.vector_points[0].x) / rx, 2) + Math.pow((cy - this.vector_points[0].y) / ry, 2) <= 1) {
					hits.push(ellipse)
					//return 1;
				}
			}
			...
			var result = Intersection.intersectEllipsePolygon(c, rx, ry, this.vector_points)
			if (result.status == "Intersection" || this.shapebound2D.pointInPolygon(c) || Math.pow((cx - this.vector_points[0].x) / rx, 2) + Math.pow((cy - this.vector_points[0].y) / ry, 2) <= 1) {
				hits.push(ellipse)
				//return 1;
			}
